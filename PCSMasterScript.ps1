...
function Invoke-Step {
    ...
    try {
        ...
        $proc = Start-Process -FilePath "powershell.exe" `
            -ArgumentList "-ExecutionPolicy Bypass -NonInteractive -WindowStyle Hidden -File `"$OutFile`"" `
            -Wait -PassThru
...
$LegacyFlags = @(
    ...
    "C:\Users\Public\AppData\MasterScriptDone1.0.txt"
)
...
Set-Reg "ConfigStepDetail"  "Starting PCS configuration"
Set-Reg "ScriptStartTime"   (Get-Date -Format "yyyy-MM-dd HH:mm:ss")
Set-Reg "LastUpdated"       (Get-Date -Format "yyyy-MM-dd HH:mm:ss")

$TotalSteps = 6

Write-Host "PCS Master Script started at $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
...
Invoke-Step `
    ...
    -Url      "https://raw.githubusercontent.com/medteamadmins/ChangeComputerName/main/PCSChangeComputerName.ps1" `
    -OutFile  "$TMTDir\ChangeComputerName\PCSChangeComputerName.ps1"

# =====================================================================
# STEP 2: Re-Partition Disk
# =====================================================================
Write-Step 2 "Step 2/$TotalSteps: Re-Partition Disk" "Preparing"

Invoke-Step `
    -StepName "Re-Partition" `
    -Url      "https://raw.githubusercontent.com/medteamadmins/Repartition/main/Repartition.ps1" `
    -OutFile  "$TMTDir\Partition\Repartition.ps1"

# =====================================================================
# STEP 3: Folder Redirection
# =====================================================================
Write-Step 3 "Step 3/$TotalSteps: Folder Redirection" "Preparing"

Invoke-Step `
    -StepName "Folder Redirection" `
    -Url      "https://raw.githubusercontent.com/medteamadmins/FolderRedirection/main/FolderRedirectionDownload.ps1" `
    -OutFile  "$TMTDir\FolderRedirection\FolderRedirectionDownload.ps1"

# =====================================================================
# STEP 4: Remove HP Bloatware
# =====================================================================
Write-Step 4 "Step 4/$TotalSteps: Remove HP Bloatware" "Preparing"
...
# =====================================================================
Write-Step 5 "Step 5/$TotalSteps: Remove Windows Bloatware" "Preparing"
...
# =====================================================================
# STEP 6: Wait for All Applications
# =====================================================================
Write-Step 6 "Step 6/$TotalSteps: Waiting for Applications" "Checking installed apps"
...
# =====================================================================
# CREATE SHORTCUTS (apps confirmed installed above)
# =====================================================================
Write-Detail "Creating desktop shortcuts"
$WS = New-Object -ComObject WScript.Shell

# Word
Copy-Item "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Word.lnk" `
          $PubDesk -Force -ErrorAction SilentlyContinue
Write-Detail "Word shortcut done"

# Excel
Copy-Item "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Excel.lnk" `
          $PubDesk -Force -ErrorAction SilentlyContinue
Write-Detail "Excel shortcut done"

# Google Chrome
$sc = $WS.CreateShortcut("$PubDesk\Google Chrome.lnk")
$sc.TargetPath = "C:\Program Files\Google\Chrome\Application\chrome.exe"
$sc.Save()
Write-Detail "Chrome shortcut done"

# Adobe Acrobat
$AcrobatPath = if (Test-Path "C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe") {
    "C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe"
} else {
    "C:\Program Files (x86)\Adobe\Acrobat Reader DC\Reader\AcroRd32.exe"
}
$sc = $WS.CreateShortcut("$PubDesk\Adobe Acrobat.lnk")
$sc.TargetPath = $AcrobatPath
$sc.Save()
Write-Detail "Adobe shortcut done"
...
Set-Reg "ConfigStepDetail" "PCS configuration finished successfully"
Set-Reg "ConfigComplete"   1
Set-Reg "LastUpdated"      (Get-Date -Format "yyyy-MM-dd HH:mm:ss")

Write-Host "PCS Master Script completed successfully at $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
exit 0
